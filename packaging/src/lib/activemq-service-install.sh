...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]] && [[ "${BASH_SOURCE[0]}" == *"activemq-service-install.sh" ]]; then
    action="${1:-}"
    user="${2:-activemq}"
    ...
    case "$action" in
        ...
        verify)
            ...
            ;;
        help|*)
            echo "Usage: $0 [install|update|verify] [user]"
            echo ""
            echo "Actions:"
            echo "  install  - Install ActiveMQ configuration from templates"
            echo "  update   - Update existing configuration"
            echo "  verify   - Verify configuration health"
            echo ""
            echo "Environment variables:"
            echo "  ACTIVEMQ_BROKER_NAME    - Broker name (default: localhost)"
            echo "  ACTIVEMQ_BIND_ADDRESS   - Bind address (default: 127.0.0.1)"
            echo "  ACTIVEMQ_PORT          - OpenWire port (default: 61616)"
            echo "  ACTIVEMQ_STOMP_PORT    - STOMP port (default: 61613)"
            echo "  ACTIVEMQ_LOG_LEVEL     - Log level (default: INFO)"
            echo "  ACTIVEMQ_PITRAC_PASSWORD - PiTrac user password"
            exit 1
            ;;
    esac
fi